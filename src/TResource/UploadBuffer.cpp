...
void UploadBuffer::Create(const std::wstring& name, size_t BufferSize)
{
	...
	// createa an upload buffer. This is CPU-visible, but it's write combined memory, so avoid reading back from it.
	...
	ThrowIfFailed(TD3D12RHI::g_Device->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE, &ResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_pResource)));
