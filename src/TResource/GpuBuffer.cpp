...
void GpuBuffer::Create(const std::wstring& name, uint32_t NumElements, uint32_t ElementSize, const void* initialData)
{
    ...
    m_UsageState = D3D12_RESOURCE_STATE_COMMON; // state for default heap
    ...
    ThrowIfFailed(TD3D12RHI::g_Device->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE, &ResourceDesc, m_UsageState, nullptr, IID_PPV_ARGS(&m_pResource)));
...
void GpuBuffer::CreatePlaced(const std::wstring& name, ID3D12Heap* pBackingHeap, uint32_t HeapOffset, uint32_t NumElements, uint32_t ElementSize, const void* initialData)
{
    ...
    ThrowIfFailed(TD3D12RHI::g_Device->CreatePlacedResource(pBackingHeap, HeapOffset, &ResourceDesc, m_UsageState, nullptr, IID_PPV_ARGS(&m_pResource)));
...
D3D12_CPU_DESCRIPTOR_HANDLE GpuBuffer::CreateConstantBufferView(uint32_t Offset, uint32_t Size) const
{
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE hCBV = TD3D12RHI::SRVHeapSlotAllocator->AllocateHeapSlot().Handle;
    TD3D12RHI::g_Device->CreateConstantBufferView(&CBVDesc, hCBV);
...
void ByteAddressBuffer::CreateDerivedViews()
{
    ...
    SRVDesc.Buffer.NumElements = (UINT)m_BufferSize / 4; //byte*
    SRVDesc.Buffer.Flags = D3D12_BUFFER_SRV_FLAG_RAW;

    if (m_SRV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
        m_SRV = TD3D12RHI::SRVHeapSlotAllocator->AllocateHeapSlot().Handle;

    TD3D12RHI::g_Device->CreateShaderResourceView(m_pResource.Get(), &SRVDesc, m_SRV);
    ...
    TD3D12RHI::g_Device->CreateUnorderedAccessView(m_pResource.Get(), nullptr, &UAVDesc, m_UAV);
}

void StructureBuffer::CreateDerivedViews()
{
    D3D12_SHADER_RESOURCE_VIEW_DESC SRVDesc = {};
    SRVDesc.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
    ...
    TD3D12RHI::g_Device->CreateUnorderedAccessView(m_pResource.Get(), m_CounterBuffer.GetResource(), &UAVDesc, m_UAV);
}
